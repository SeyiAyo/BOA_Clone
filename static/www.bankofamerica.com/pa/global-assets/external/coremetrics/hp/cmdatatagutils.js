...
cmMakeTag(["tid","4","at","5","pr",prID,"pm",prName,"qt",prQty,"bp","1","cg",catID,"sx3",state,"sx14",prType,"sx15",sPF,"cm_exAttr",cmAttr(attr)]);  	
   deleteCookie("OrderIDcookie","../index.html");
}
...
function cmMakeTag(__v) {
	...
	if (cm.tid=="6" && cI("cmTPSet")!='Y') {  
		if (window.location.href.toLowerCase().indexOf("bankofamerica.com") > -1) 
			document.cookie = "cmTPSet%3dY%3b%20domain%3dbankofamerica.com%3b%20path%3d/index.html";
		else 
			document.cookie = "cmTPSet%3dY%3b%20path%3d/index.html";
...
function autoOrderID()
{
  if ((getCookie("OrderIDcookie")==undefined)||((getCookie("OrderIDcookie"))&&(getCookie("OrderIDcookie")==""))) {
    ...
    setCookie("OrderIDcookie",orderID,"../index.html");
    return orderID;
  }
...
function cmGetAdString(){
	...
	for (i = 0; i < linkCt; i++) {
		...
		ndx2 = lurl.lastIndexOf("https://www.bankofamerica.com/adtrack/");
		if ((ndx >= 0) && (ndx2>0)) {
	...
	document.cmTagCtl.normalizeURL = function(url, isHref) { 
		...
		try {
			...
			if (isHref) { 
				...
				if (newURL.indexOf("../cid/index.html")>-1) {
					cmN = "/cid/;" + cmN; 
				}

				if (newURL.indexOf("../ias/index.html")>-1) {
					cmN = "/ias/;" + cmN; 
				}
			}
